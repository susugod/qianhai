(function ($) {
    ...
    $.confirm = function (options, e) {
        ...
        if (options.confirmButton != null) {
            if (options.cancelButton != null) {
                ...
                if (options.mousePosition) {
                    modalConfirmBtn =
                    '<button class="confirm btn btn-mini ' + settings.confirmButtonClass + '" type="button" data-dismiss="modal" style="height:30px;width:40px; padding:0px;position:relative;bottom:8px;right:-10px;margin-right:10px;">' +
                        settings.confirmButton +
                    '</button>';
                }
                else {
            ...
            modalHTML =
                    '<div class="confirmation-modal modal fade" tabindex="-1" role="dialog">' +
                        '<div class="modal-dialog" style="z-index:9999; position:absolute;">' +
                            '<div class="modal-content" style="width:300px; left:' + newX + 'px; top:' + newY + 'px;">' +
                                modalHeader +
                                modalBody +
                                modalFooter +
                            '</div>' +
        ...
        modal.on('shown.bs.modal', function () {
            if ($(parent.document.body).find('#iboAlert').length > 0)
                $(parent.document.body).find('#iboAlert').remove();
        });
    ...
    var o_html = '<div id="iboAlert" hidden="hidden"></div>';
    var o_alert = null;

    iboMousePositionAlert = function (message, title) { //alert框的位置随着鼠标点击的位置不同而不同
        var alertParams = {
            text: message,
            title: title,
            mousePosition: true,
    ...
    iboAlertWarning = function (message, title) {
        var alertParams = {
            text: message,
            title: title,
            style: 'AlertWarning',
    ...
    iboAlertError = function (message, title) {
        var alertParams = {
            text: message,
            title: title,
            style: 'AlertError',
    ...
    iboAlertSuccess = function (message, callback, title) {
        var alertParams;
        if (callback != null) {
            alertParams = {
                text: message,
    ...
    iboConfirm = function (message, callback, title) {
        var alertParams = {
            text: message,
            title: title,
            controlButton: true,
    ...
    $.ProcessAlert = function(alertParams) {
        if ($(parent.document.body).find("#iboAlert").length < 1) {
            $(parent.document.body).append($(o_html));
        }

        o_alert = $(parent.document.body).find("#iboAlert");
